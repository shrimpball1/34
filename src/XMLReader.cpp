...
struct CXMLReader::SImplementation{
    ...
    bool ReadEntity(SXMLEntity &entity, bool skipcdata){
        ...
        SXMLEntity current = DEntities.front();
        DEntities.pop();
        // if the current entity is CharData, combine all CharData entities
        if (current.DType == SXMLEntity::EType::CharData) {
            while (!DEntities.empty() && DEntities.front().DType == SXMLEntity::EType::CharData) {
                current.DNameData += DEntities.front().DNameData;
                DEntities.pop();
            }
        }
